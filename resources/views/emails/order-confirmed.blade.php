...
<div style="background-color: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
    ...
    <table style="width: 100%; margin: 20px 0;" cellpadding="0" cellspacing="0">
        <tr>
            <td style="width: 50%; vertical-align: top; padding-right: 10px;">
                <p style="margin: 0 0 5px 0; font-weight: bold; color: #333;">Dirección de entrega:</p>
                <p style="margin: 0; color: #333;">{{ $order->customer_name }}</p>
                <p style="margin: 0; color: #333;">{{ $order->shipping_address }}</p>
                <p style="margin: 0; color: #333;">{{ $order->shipping_city }}, {{ $order->shipping_state }} {{ $order->shipping_zip }}</p>
                <p style="margin: 0; color: #333;">{{ $order->shipping_country }}</p>
            </td>
            <td style="width: 50%; vertical-align: top; padding-left: 10px;">
                <p style="margin: 0 0 5px 0; font-weight: bold; color: #333;">Medio de pago:</p>
                <p style="margin: 0; color: #333;">{{ ucfirst($order->payment_method) }}</p>
            </td>
        </tr>
    </table>
    ...
    <div style="margin-top: 30px; color: #333;">
        <p style="margin: 0 0 15px 0; color: #333; line-height: 1.6;">En cuanto tu pedido esté listo para envío, te lo notificaremos por correo con el número de seguimiento.</p>
